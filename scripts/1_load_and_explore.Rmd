...
# 3. Simple descriptive statistics: TUMULUS example
...
```{r}
dated <- tumulus %>% 
  select(`EDCS-ID`, date_not_before,date_not_after) 
dated
```
...
```{r}
#install.packages("ggalt") #uncomment and run if the package is not installed on your computer
library(ggalt)

dated %>% ggplot( aes(x = date_not_before, xend = date_not_after, y = `EDCS-ID`)) +
  geom_dumbbell(colour = "#a3c4dc",colour_xend = "#0e668b", size = 1 ) +
